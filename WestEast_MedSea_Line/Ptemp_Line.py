...
# def get_array_mean():
#     nc_file = 'D:/WWLLN-Intensity/Validation CSV/info/ptemp.nc'
        ...
        for i in pandas_times:
            if year == i.year:
                ...
                if i.month in [1, 2, 3]:
                    ptimes_list = pandas_times.to_list()
                    index = ptimes_list.index(i)
                    alk_stime = ptemp_ds.thetao.isel(time=index)
...
def export_ptemp_on_line(long_points, ptemp_data):
    ...
    df['PTemp'] = ptemp_data
    return df
    # df2.to_csv('D:/WWLLN-Intensity/Validation CSV/info/data/ptemp_on_line.csv')
...
def main():
    ...
    writer = pd.ExcelWriter('D:/WWLLN-Intensity/Validation CSV/info/data/ptemp_on_line.xlsx', engine='xlsxwriter')
    for year in years_array_dict:
        year_array = years_array_dict[year]
        alk_data = get_ptemp_values(long_index, lat_index, year_array)
        df = export_ptemp_on_line(long_points, alk_data)
        df = df[(df.longs >= 16.5)]
