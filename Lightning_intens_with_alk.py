...
def get_energy_plot_mean_total(total_array_mean, ax):
    long_points_med, lat_points_med, islands_dict = get_long_lats_med()
    ...
    imshow_mean_total = ax.imshow(total_mean.T, origin='lower', cmap=cmap, alpha=0.8,  extent=[min(long_points_med), max(long_points_med), min(lat_points_med), max(lat_points_med)])
...
def main():
    ...
    cmap.set_under('w')
    alk_plot = plt.pcolormesh(long_list, lat_list, mean_array[0], cmap= cmap, shading='gouraud', zorder=0)
    ax = plt.gca()
    get_energy_plot_mean_total(total_array_mean, ax)
